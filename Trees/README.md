...
| 4 | [226. Invert Binary Tree](https://leetcode.com/problems/invert-binary-tree/) | 3m logic<br> 4m total | **Pattern:** Post-Order Traversal <br>**Trigger:** "invert a binary tree" | **Time:** `O(n)` <br>**Space:** `O(h)` | Invert left subtree and right subtree and then invert the root | For root as null root of inverted tree with also be null |
| 5 | [110. Balanced Binary Tree](https://leetcode.com/problems/balanced-binary-tree/) | 2m logic<br> 5m total | **Pattern:** Post Order Traversal <br>**Trigger:** "check if a binary tree is height-balanced" | **Time:** `O(n)` <br>**Space:** `O(h)` | Check if left subtree and right subtree are height balanced and then check root | For root as null binary tree will be height balanced |